...
export function gregorianToBuddhist(d) {
  return {
    year: d.getUTCFullYear() + BUDDHIST_OFFSET,
    month: d.getUTCMonth() + 1,
    day: d.getUTCDate()
  };
}
export function buddhistToGregorian(year, month, day) {
...
function isLeapFixed13(year) {
  return (year % 4 + 4) % 4 === 3;
}
function firstDayOfYearFixed13(year, pivotYear) {
  const relativeYear = year - pivotYear;
  const leapYears = relativeYear <= 0 ? Math.floor((relativeYear + 3) / 4) : Math.floor(relativeYear / 4) + (isLeapFixed13(year) ? 0 : 1);
  return relativeYear * 365 + leapYears + FIXED13_EPOCH_DAY_OFFSET;
}
function daysInMonthFixed13(year, month) {
  return month !== 13 ? 30 : isLeapFixed13(year) ? 6 : 5;
}
function gregorianToFixed13(d, pivotYear) {
...
function fixed13ToGregorian(year, month, day, pivotYear) {
...
export function gregorianToCoptic(d) {
...
export function gregorianToEthiopian(d) {
...
function islamicFirstDayOfYear(year) {
  const y = year - 1;
...
export function gregorianToIslamic(d) {
  const epochDay = dateToEpochDay(d);
...
export function islamicToGregorian(year, month, day) {
...
const PERSIAN_BREAK_YEARS = [-61, 9, 38, 199, 426, 686, 756, 818, 1111, 1181, 1210, 1635, 2060, 2097, 2192, 2262, 2324, 2394, 2456, 3178];
function persianBreakYearCalc(persianYear, calcJalaaliLeaps) {
  let jalaaliLeaps = -14;
  let jump = 0;
...
function persianIsLeapYear(year) {
  const {
    jump
  } = persianBreakYearCalc(year, false);
  let {
    diff
  } = persianBreakYearCalc(year, false);
  if (jump - diff < 6) {
...
function persianFirstDayOfYear(persianYear) {
  const {
    jump,
    diff,
    jalaaliLeaps
  } = persianBreakYearCalc(persianYear, true);
...
export function gregorianToPersian(d) {
  const epochDay = dateToEpochDay(d);
