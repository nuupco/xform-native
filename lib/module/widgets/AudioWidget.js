"use strict";

/**
 * AudioWidget — binary audio capture/playback (REQ-M11..M14).
 *
 * Gated on expo-audio (optional peer dep). Falls back to
 * UnsupportedWidget when the dep is absent at runtime.
 *
 * Restyle (design decision 7, per-widget mapping table, PR12): chrome moves
 * onto `MediaCaptureCard`. `state:'captured'` has no natural preview for
 * audio (no visual media), so — following BarcodeWidget's precedent (PR11)
 * of reusing the `preview` slot as a generic "custom content" region — the
 * "recording exists" case renders a `MicIcon` + label through `preview`
 * while it still surfaces the Play action. `active` covers the
 * in-progress-recording state (`MicIcon` pulsing via `recordingPulse`,
 * `Stop` action with `tone:'error'`).
 *
 * expo-audio migration (urgent SDK-56 fix, expo-av dropped): expo-audio's
 * API is hook-based (`useAudioRecorder`/`useAudioPlayer`), not the old
 * imperative `new Audio.Recording()` class. Both hooks are called
 * unconditionally on every render, reached through the optional-chained
 * lazily-`require`d module reference (`av?.useAudioRecorder?.(...)`) so
 * whether the peer is present is decided once at module-require time and
 * never varies across a given component instance's lifetime — preserving
 * hooks-rules safety together with the optional-peer-dep firewall (mirrors
 * the file's existing `useThemedStyles`-must-stay-first invariant). When
 * `av` is absent, the hooks receive dummy fallback args behind `?.` and are
 * never reached in practice because `getAudioModule()` fails to `require`
 * eagerly; the `useState`-based fallback fields on the returned recorder are
 * only for the type-level `?? {}` no-op guard below.
 */
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { Animated, Text, StyleSheet } from 'react-native';
import { UnsupportedWidget } from "./UnsupportedWidget.js";
import { MediaCaptureCard } from "./primitives/MediaCaptureCard.js";
import { MicIcon } from "./primitives/Icon.js";
import { useThemedStyles } from "../theme/ThemeContext.js";
import { usePermissionGate, isPermissionBlockingState } from "./primitives/usePermissionGate.js";
import { PermissionNotice } from "./primitives/PermissionNotice.js";
import { jsx as _jsx, Fragment as _Fragment, jsxs as _jsxs } from "react/jsx-runtime";
let _AudioModule = null;
let _avLoaded;
function getAudioModule() {
  if (_avLoaded === undefined) {
    try {
      _AudioModule = require('expo-audio');
      _avLoaded = true;
    } catch {
      _avLoaded = false;
    }
  }
  return _AudioModule;
}
function createStyles(t) {
  return StyleSheet.create({
    savedReadout: {
      ...t.typography.bodyMedium,
      color: t.color.roles.onSurface
    }
  });
}
function getMicNoticeCopy(status) {
  switch (status) {
    case 'rationale':
      return {
        title: 'Usar el micrófono',
        body: 'Necesitamos el micrófono para grabar tu respuesta de audio.'
      };
    case 'blocked':
      return {
        title: 'Permiso de micrófono bloqueado',
        body: 'Actívalo en los ajustes del sistema para grabar audio.'
      };
    case 'error':
      return {
        title: 'No se pudo iniciar la grabación',
        body: 'Vuelve a intentarlo.'
      };
    case 'denied':
    default:
      return {
        title: 'Sin permiso de micrófono',
        body: 'Permite el acceso para grabar audio.'
      };
  }
}
export function AudioWidget({
  nodeRef,
  store,
  appearance: _appearance
}) {
  // Theming (D2): useThemedStyles MUST stay the first statement, before the
  // peer-dependency gating early return below.
  const styles = useThemedStyles(createStyles);
  const av = getAudioModule();
  const resolved = store.adapter.resolveValue(nodeRef);
  const storedUri = typeof resolved === 'string' && resolved.length > 0 ? resolved : null;
  const nodeState = store.adapter.getNodeState(nodeRef);
  const readonly = nodeState.readonly;
  const [isRecording, setIsRecording] = useState(false);
  const isRecordingRef = useRef(false);

  // Hooks-rules-safe unconditional call through the lazily-`require`d
  // module reference: `av`'s truthiness is decided once at module-require
  // time and never varies across this component instance's lifetime, so
  // calling through `?.` here is safe (mirrors this file's
  // `useThemedStyles`-first invariant). When the peer is absent, `av` is
  // `null` and both calls short-circuit to `undefined` without invoking a
  // hook.
  const recorder = av?.useAudioRecorder?.(av?.RecordingPresets?.HIGH_QUALITY ?? {});
  const player = av?.useAudioPlayer?.(storedUri ? {
    uri: storedUri
  } : null);
  const micAdapter = useMemo(() => ({
    get: () => av?.getRecordingPermissionsAsync?.(),
    request: () => av?.requestRecordingPermissionsAsync?.()
  }), [av]);
  const gate = usePermissionGate(micAdapter);

  // Recording pulse — one-shot opacity dip (not a repeating loop: the global
  // `Animated.timing` jest stub resolves `start()` synchronously, which would
  // recurse forever through `Animated.loop`, see PR7's gotcha). Mirrors
  // RankWidget's lift animation shape: animate to a value on state change.
  const pulseOpacity = useRef(new Animated.Value(1)).current;
  useEffect(() => {
    Animated.timing(pulseOpacity, {
      toValue: isRecording ? 0.5 : 1,
      duration: isRecording ? 500 : 200,
      useNativeDriver: true
    }).start();
  }, [isRecording, pulseOpacity]);
  const handleRecord = useCallback(async () => {
    if (!av || !recorder || readonly) return;
    if (!(await gate.ensure())) return;
    try {
      await recorder.prepareToRecordAsync();
      recorder.record();
      isRecordingRef.current = true;
      setIsRecording(true);
    } catch {
      // Genuine runtime failure (device busy, hardware issue) despite a
      // granted permission — design decision 9. Never leave this as an
      // unhandled rejection.
      gate.markError();
      isRecordingRef.current = false;
      setIsRecording(false);
    }
  }, [av, recorder, readonly, gate]);
  const handleStop = useCallback(async () => {
    if (!av || !recorder || readonly || !isRecordingRef.current) return;
    try {
      await recorder.stop();
      const uri = recorder.uri;
      if (uri) {
        store.answerQuestion(nodeRef, uri);
      }
    } catch {
      // best-effort; nothing else to do on a failed stop
    } finally {
      isRecordingRef.current = false;
      setIsRecording(false);
    }
  }, [av, recorder, readonly, nodeRef, store]);
  const handlePlay = useCallback(() => {
    if (!av || !player || readonly) return;
    if (!storedUri) return;
    try {
      player.play();
    } catch {
      // best-effort; playback failure has no dedicated UI state
    }
  }, [av, player, readonly, storedUri]);
  useEffect(() => {
    return () => {
      if (av && recorder && isRecordingRef.current) {
        try {
          recorder.stop();
        } catch {
          // best-effort cleanup on unmount
        }
        isRecordingRef.current = false;
      }
      if (av && player) {
        try {
          player.remove?.();
        } catch {
          // best-effort cleanup on unmount
        }
      }
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  if (!av) {
    return /*#__PURE__*/_jsx(UnsupportedWidget, {
      dataType: "binary"
    });
  }
  if (!readonly && !isRecording && (isPermissionBlockingState(gate.status) || gate.status === 'error')) {
    const copy = getMicNoticeCopy(gate.status);
    return /*#__PURE__*/_jsx(MediaCaptureCard, {
      testID: "audio-widget",
      state: "captured",
      icon: /*#__PURE__*/_jsx(MicIcon, {}),
      title: "No recording",
      actions: [],
      preview: /*#__PURE__*/_jsx(PermissionNotice, {
        title: copy.title,
        body: copy.body,
        primaryLabel: gate.status === 'blocked' ? 'Abrir ajustes' : 'Permitir',
        onPrimary: gate.status === 'blocked' ? gate.openSettings : gate.requestPermission,
        dismissLabel: gate.status === 'blocked' ? undefined : 'Ahora no',
        onDismiss: gate.status === 'blocked' ? undefined : gate.dismissRationale
      })
    });
  }
  if (isRecording) {
    return /*#__PURE__*/_jsx(MediaCaptureCard, {
      testID: "audio-widget",
      state: "active",
      icon: /*#__PURE__*/_jsx(Animated.View, {
        testID: "audio-recording-pulse",
        style: {
          opacity: pulseOpacity
        },
        children: /*#__PURE__*/_jsx(MicIcon, {})
      }),
      title: "Recording\u2026",
      disabled: readonly,
      actions: [{
        label: 'Stop',
        onPress: handleStop,
        tone: 'error',
        testID: 'audio-stop-button'
      }]
    });
  }
  if (storedUri) {
    return /*#__PURE__*/_jsx(MediaCaptureCard, {
      testID: "audio-widget",
      state: "captured",
      icon: /*#__PURE__*/_jsx(MicIcon, {}),
      title: "Recording saved",
      disabled: readonly,
      preview: /*#__PURE__*/_jsxs(_Fragment, {
        children: [/*#__PURE__*/_jsx(MicIcon, {}), /*#__PURE__*/_jsx(Text, {
          style: styles.savedReadout,
          children: "Recording saved"
        })]
      }),
      actions: [{
        label: 'Play',
        onPress: handlePlay,
        testID: 'audio-play-button'
      }]
    });
  }
  return /*#__PURE__*/_jsx(MediaCaptureCard, {
    testID: "audio-widget",
    state: "empty",
    icon: /*#__PURE__*/_jsx(MicIcon, {}),
    title: "No recording",
    disabled: readonly,
    actions: [{
      label: 'Record',
      onPress: handleRecord,
      testID: 'audio-record-button'
    }]
  });
}
//# sourceMappingURL=AudioWidget.js.map