...
import { parseOdkMarkdown } from "./parseOdkMarkdown.js";
...
function isFastPath(blocks) {
  if (blocks.length !== 1) return false;
  const block = blocks[0];
  if (block.headerLevel !== 0) return false;
  if (block.spans.length !== 1) return false;
  const span = block.spans[0];
  return span.bold === undefined && span.italic === undefined && span.color === undefined && span.fontSize === undefined;
}
function renderBlock(block, blockIndex) {
  const spanNodes = block.spans.map((span, spanIndex) => /*#__PURE__*/_jsx(Text, {
    style: spanStyle(span),
    children: span.text
  }, spanIndex));
  if (block.headerLevel > 0) {
    return /*#__PURE__*/_jsx(Text, {
      style: headerStyle(block.headerLevel),
      children: spanNodes
    }, blockIndex);
  }
