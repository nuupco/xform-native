...
import { pickBest } from "../widgets/engine/matchOverride.js";
 ...
 */
export const defaultAdvanceValidator = ctx => {
  const {
    nodeRef,
    store,
    event
  ...
  if (lastResult?.ref === event.ref && lastResult.result === AnswerResult.CONSTRAINT_VIOLATED) {
    return {
      type: 'constraint',
 ...
 */
export function resolveValidator(event, overrides) {
...
const ValidatorRegistryContext = /*#__PURE__*/createContext([]);
export function ValidationRegistryProvider(p) {
  return /*#__PURE__*/createElement(ValidatorRegistryContext.Provider, {
    value: p.validators
  }, p.children);
}
export function useValidatorOverrides() {
