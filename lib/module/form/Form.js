...
import { resolveWidget, useWidgetOverrides } from "../widgets/engine/registry.js";
import { useThemedStyles } from "../theme/ThemeContext.js";
import { BofSurface, EofSurface, ConstraintSurface, RequiredSurface, LabelHint, RepeatPromptCard, WidgetErrorFallback } from "./surfaces.js";
...
import { renderSlot, INJECT_VALUES_APPEARANCE } from "./slots.js";
import { defaultAdvanceValidator, resolveValidator, useValidatorOverrides } from "./validation.js";
 ...
 */
function isDirectChildOf(childRef, groupRef) {
...
function isDescendantOf(ref, groupRef) {
 ...
 */
function scanDirectQuestions(store, containerRef, state, firstEv) {
  ...
  for (;;) {
    ...
    if ((ev.kind === 'group' || ev.kind === 'repeat' || ev.kind === 'prompt-new-repeat') && isDescendantOf(ev.ref, containerRef)) {
      return {
        questions,
        aborted: true,
        next: ev
 ...
 */
function buildRepeatSection(store, firstEv, containerRef, state) {
  const instances = [];
...
function scanFieldListBlocks(store, containerRef, state, firstEv) {
  const blocks = [];
  let pending = firstEv;
  for (;;) {
    const scan = scanDirectQuestions(store, containerRef, state, pending);
    pending = undefined;
    for (const q of scan.questions) blocks.push({
      kind: 'question',
      event: q
    });
    if (!scan.aborted) {
    ...
    const ev = scan.next;
    if (ev.kind === 'group' && isDirectChildOf(ev.ref, containerRef) && ev.appearance === 'field-list') {
      ...
      for (const b of nested.blocks) {
        if (b.kind === 'question') nestedQuestions.push(b.event);else {
          deepOk = false;
          break;
        }
      }
    ...
    if ((ev.kind === 'repeat' || ev.kind === 'prompt-new-repeat') && isDirectChildOf(ev.ref, containerRef)) {
...
function planFieldList(store, group) {
  const state = {
    steps: 0
  };
...
function flattenPlanQuestions(blocks) {
  const out = [];
  for (const b of blocks) {
    if (b.kind === 'question') out.push(b.event);else if (b.kind === 'group-header') out.push(...b.questions);else for (const inst of b.instances) out.push(...inst.questions);
  }
  return out;
}
/**
 ...
 */
function planInjectValues(store, group) {
  const adapter = store.adapter;
  ...
  if (sawInvalidNesting) {
    console.error(`[xform-native] group "${fieldNameOf(containerRef)}" has appearance="${INJECT_VALUES_APPEARANCE}" ` + 'but is not flat — it contains a nested group/repeat. inject-values groups must contain only ' + 'direct question children; the nested content was skipped. Nested repeats are not supported.');
  }
  return {
    fields,
    totalSteps: steps
...
export function Form({
  ...
  const overridesRef = useRef([...contextOverrides, ...(widgetsProp ?? [])]);
  ...
  const validatorOverridesRef = useRef([...contextValidatorOverrides, ...(validatorsProp ?? [])]);
  ...
  const [fieldListBlocked, setFieldListBlocked] = useState(null);
  ...
  useEffect(() => {
    ...
    const navigable = ev.kind === 'question' || ev.kind === 'group' || ev.kind === 'repeat' || ev.kind === 'prompt-new-repeat';
    ...
    if (!isInjectValues && (ev.kind === 'group' || ev.kind === 'repeat') && (ev.label === null || ev.label === '')) {
      step();
      return;
    }
  ...
  const fieldListPlan = event.kind === 'group' && event.appearance === 'field-list' ? planFieldList(store, event) : null;
  const fieldListPlanRef = useRef(null);
  ...
  const injectValuesPlan = event.kind === 'group' && event.appearance === INJECT_VALUES_APPEARANCE ? planInjectValues(store, event) : null;
  const injectValuesPlanRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (prev && prev.index !== eventIndex) {
      ...
      const wasAnswered = lastResult !== null && lastResult.ref === prev.ref && lastResult.result === AnswerResult.OK;
      progressRef.current.set(prev.index, wasAnswered ? 'answered' : 'skipped');
    }
    prevQuestionRef.current = event.kind === 'question' ? {
      ref: event.ref,
      index: event.index
    } : null;
  ...
  const handleNext = useCallback(() => {
    ...
    if (ev.kind === 'question') {
      ...
      const validate = resolveValidator(ev, validatorOverridesRef.current) ?? defaultAdvanceValidator;
      const block = validate({
        nodeRef: ev.ref,
        store,
        event: ev,
        defaultValidate: () => defaultAdvanceValidator({
          nodeRef: ev.ref,
          store,
          event: ev,
      ...
      for (const q of questions) {
        const validate = resolveValidator(q, validatorOverridesRef.current) ?? defaultAdvanceValidator;
        const block = validate({
          nodeRef: q.ref,
          store,
          event: q,
  ...
  const handleAddRepeatInstance = useCallback(ref => {
    store.createRepeatInstance(ref);
  }, [store]);
  const handleRemoveRepeatInstance = useCallback(ref => {
  ...
  function renderQuestionField(ev, block) {
    const nodeState = store.adapter.getNodeState(ev.ref);
    const {
      Widget
    } = resolveWidget({
    ...
    return /*#__PURE__*/_jsxs(View, {
      ...
      children: [/*#__PURE__*/_jsx(LabelHint, {
        ...
        style: block != null ? styles.widgetSlotError : undefined,
        collapsable: false,
        children: /*#__PURE__*/_jsx(WidgetErrorBoundary, {
          fallback: /*#__PURE__*/_jsx(WidgetErrorFallback, {
            fieldName: fieldNameOf(ev.ref)
          }),
      ...
      }), block && renderSlot(slots?.renderError, {
        block,
        defaultElement: block.type === 'required' ? /*#__PURE__*/_jsx(RequiredSurface, {}) : /*#__PURE__*/_jsx(ConstraintSurface, {
          message: block.message
        })
      })]
    }, ev.index);
  }
  function renderFieldListBlock(block, key) {
    ...
    if (block.kind === 'group-header') {
      return /*#__PURE__*/_jsxs(View, {
        collapsable: false,
        style: styles.fieldListSubGroup,
        children: [/*#__PURE__*/_jsx(LabelHint, {
          label: block.label,
          hint: block.hint
    ...
    return /*#__PURE__*/_jsxs(View, {
      collapsable: false,
      style: styles.fieldListSubGroup,
      children: [block.instances.map((inst, i) => /*#__PURE__*/_jsxs(View, {
        collapsable: false,
        style: styles.repeatInstanceCard,
        children: [/*#__PURE__*/_jsx(LabelHint, {
          label: `${block.label ?? ''} ${i + 1}`.trim(),
          hint: null
        }), inst.questions.map(q => renderQuestionField(q, fieldListBlocked?.get(q.index) ?? null)), /*#__PURE__*/_jsx(PressableButton, {
      ...
      }, i)), block.createRef !== null && /*#__PURE__*/_jsx(RepeatPromptCard, {
        label: block.label ?? '',
        onPress: () => handleAddRepeatInstance(block.createRef)
      })]
    }, key);
  }
    ...
    switch (ev.kind) {
      ...
      case 'eof':
        {
          ...
          for (const outcome of progressRef.current.values()) {
            if (outcome === 'answered') answered += 1;else skipped += 1;
          }
      ...
      case 'group':
        ...
        return renderSlot(slots?.renderGroup, {
          event: ev,
          defaultElement: fieldListPlan !== null ? /*#__PURE__*/_jsxs(View, {
            collapsable: false,
            children: [/*#__PURE__*/_jsx(LabelHint, {
              label: ev.label,
              hint: ev.hint
      ...
      case 'repeat':
        ...
        return renderSlot(slots?.renderGroup, {
          event: ev,
          defaultElement: /*#__PURE__*/_jsx(View, {
            collapsable: false,
            children: /*#__PURE__*/_jsx(LabelHint, {
              label: ev.label,
              hint: null
      ...
      case 'prompt-new-repeat':
        return /*#__PURE__*/_jsxs(View, {
          ...
          children: [/*#__PURE__*/_jsx(LabelHint, {
            label: ev.label,
            hint: null
          }), /*#__PURE__*/_jsx(RepeatPromptCard, {
            label: ev.label ?? '',
            onPress: handleCreateRepeat
          })]
        });
      default:
        return null;
  ...
  return /*#__PURE__*/_jsxs(View, {
    ...
    children: [/*#__PURE__*/_jsxs(ScrollView, {
      ...
      children: [showNav && renderSlot(slots?.renderSectionIndicator, {
        path: sectionPath,
        defaultElement: /*#__PURE__*/_jsx(SectionIndicator, {
          path: sectionPath
        })
      }), renderContent()]
    }), showNav && renderSlot(slots?.renderNavigation, {
      onBack: handleBack,
      onNext: handleNext,
      defaultElement: /*#__PURE__*/_jsx(NavRow, {
        onBack: handleBack,
        onNext: handleNext
      })
    })]
