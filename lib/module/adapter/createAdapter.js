...
export function createAdapter(session) {
  ...
  function adaptCurrentEvent() {
    ...
    if (ev.kind === 'question') {
      ...
      return {
        ...
        appearance: null,
        // FormElement.appearance not exposed via navigator; null in P1
        label: q?.getLabelInnerText() ?? null,
        hint: null,
        // FormElement.hintText not exposed via navigator; null in P1
        index: stepCount
      };
    }
    if (ev.kind === 'group') {
    ...
    if (ev.kind === 'repeat') {
      ...
      const multiplicity = lastLevel?.instanceIndex ?? 0;
      return {
        kind: 'repeat',
        ref,
        label: q?.getLabelInnerText() ?? null,
  ...
  return {
    ...
    answerQuestion(ref, value) {
      return evaluator.answerQuestion(ref, value);
    },
    resolveValue(ref) {
