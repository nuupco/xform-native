...
describe('resolveButtonBackground / resolveButtonTextColor (pure)', () => {
  it('filled + primary tone → primary bg / onPrimary text', () => {
    expect(resolveButtonBackground('filled', 'primary')).toBe(tokens.color.roles.primary);
    expect(resolveButtonTextColor('filled', 'primary')).toBe(tokens.color.roles.onPrimary);
  });

  it("filled + tone:'secondary' → secondary bg / onSecondary text", () => {
    expect(resolveButtonBackground('filled', 'secondary')).toBe(tokens.color.roles.secondary);
    expect(resolveButtonTextColor('filled', 'secondary')).toBe(tokens.color.roles.onSecondary);
  });
  ...
  it("variant:'text' → no background, primary text regardless of tone", () => {
    ...
    expect(resolveButtonTextColor('text', 'secondary')).toBe(tokens.color.roles.primary);
  });
});

describe('resolveContainerStyle (pure)', () => {
  const base = { variant: 'filled' as const, tone: 'primary' as const, height: 48, fullWidth: false };

  it('filled, unpressed: height, radius.md, primary bg, no opacity', () => {
    const style = resolveContainerStyle({ ...base, pressed: false, platformOS: 'ios' });
  ...
  it('text variant has no backgroundColor key', () => {
    const style = resolveContainerStyle({ ...base, variant: 'text', pressed: false, platformOS: 'ios' });
    expect(style.backgroundColor).toBeUndefined();
    expect(style.height).toBe(48);
  });

  it('iOS + pressed: opacity 0.7, background unchanged (no gray-out)', () => {
    const style = resolveContainerStyle({ ...base, pressed: true, platformOS: 'ios' });
  ...
  it('Android + pressed: no opacity change (ripple handles press feedback)', () => {
    const style = resolveContainerStyle({ ...base, pressed: true, platformOS: 'android' });
    expect(style.opacity).toBeUndefined();
    expect(style.backgroundColor).toBe(tokens.color.roles.primary);
  });
});
...
describe('PressableButton (component behavior)', () => {
  it('renders label text and invokes onPress when pressed', async () => {
    ...
    await render(<PressableButton label="Next" onPress={onPress} testID="btn-fire" />);
  ...
  it('disabled button does not invoke onPress when pressed', async () => {
    const onPress = jest.fn();
    await render(<PressableButton label="Next" onPress={onPress} disabled testID="btn-disabled" />);
  ...
  it("tone:'secondary' filled renders with roles.secondary container background", async () => {
    await render(
      <PressableButton label="Finalizar" onPress={() => {}} tone="secondary" testID="btn-finish" />,
    );
