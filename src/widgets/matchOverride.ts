 ...
 */
export function matchScore(matcher: WidgetMatcher, args: MatchArgs): number | null {
  let score = 0;
  ...
  if (matcher.appearance !== undefined) {
    const tokens =
      args.appearance != null ? args.appearance.toLowerCase().trim().split(/\s+/) : [];
 ...
 */
export function pickBest<T extends { match: WidgetMatcher }>(
  list: readonly T[],
  args: MatchArgs
): T | null {
