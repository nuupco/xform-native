...
export function useDraftValue<T>(cfg: DraftConfig<T>): DraftHandles {
  ...
  const value = isFocused
    ? (draft ?? storeString)
    : cfg.format
      ? cfg.format(storeString)
      : storeString;

  function onChangeText(text: string) {
    if (cfg.readonly) return;
    setDraft(text);
    const { committable, value: parsed } = cfg.parse(text);
    if (committable) {
      cfg.commit(parsed);
    }
  }
  ...
  return { value, isFocused, onChangeText, onFocus, onBlur };
}
