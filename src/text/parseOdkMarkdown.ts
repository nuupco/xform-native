...
function parseSpanStyle(styleAttr: string): ParsedStyle {
...
type Token =
  ...
  | { kind: 'bold-open' | 'bold-close' }
  | { kind: 'italic-open' | 'italic-close' }
  | { kind: 'span-open'; color?: string; fontSize?: number }
  | { kind: 'span-close' };
 ...
 */
function lexBlockToSpans(input: string): MarkdownSpan[] {
  ...
  while (i < n) {
    ...
    if (ch === '<') {
      ...
      if (openMatch) {
        ...
        tokens.push({ kind: 'span-open', color: style.color, fontSize: style.fontSize });
...
function hasUnmatchedOpen(
  tokens: Token[],
  openKind: 'bold-open' | 'italic-open',
  closeKind: 'bold-close' | 'italic-close'
): boolean {
  let depth = 0;
  for (const t of tokens) {
    if (t.kind === openKind) depth += 1;
    else if (t.kind === closeKind) depth -= 1;
  }
  return depth > 0;
}
...
function findClosingBold(input: string, from: number): number {
...
function findClosingUnderscore(input: string, from: number): number {
...
function findClosingSingleStar(input: string, from: number): number {
  for (let j = from; j < input.length; j++) {
    if (input[j] === '\\') {
      j += 1;
      continue;
...
function tokensToSpans(tokens: Token[]): MarkdownSpan[] {
  const spans: MarkdownSpan[] = [];
  let bold = 0;
  let italic = 0;
  let color: string | undefined;
  let fontSize: number | undefined;
  const spanStack: Array<{ color?: string; fontSize?: number }> = [];
...
function parseHeaderLevel(line: string): { level: 0 | 1 | 2 | 3 | 4 | 5 | 6; rest: string } {
  const match = /^(#{1,6}) (.*)$/.exec(line);
  if (match) {
    const level = match[1]!.length as 1 | 2 | 3 | 4 | 5 | 6;
    return { level, rest: match[2]! };
  }
  return { level: 0, rest: line };
}

export function parseOdkMarkdown(raw: string): MarkdownBlock[] {
  if (!HAS_METACHARACTER.test(raw)) {
    return [{ headerLevel: 0, spans: [{ text: raw }] }];
  }

  const lines = raw.split('\n');
  return lines.map((line) => {
    const { level, rest } = parseHeaderLevel(line);
    const spans = lexBlockToSpans(rest);
    return { headerLevel: level, spans: spans.length > 0 ? spans : [{ text: '' }] };
  });
}

export function stripOdkMarkdown(raw: string): string {
  if (!HAS_METACHARACTER.test(raw)) {
    return raw;
  }
